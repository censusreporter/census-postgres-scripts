CREATE TABLE tiger2017.aiannh252 AS (
    ...
    FROM tiger2017.aiannh
    WHERE comptyp = 'R'
);

CREATE TABLE tiger2017.aiannh254 AS (
    SELECT gid, aiannhce, aiannhns, geoid, name, namelsad, lsad, classfp,
           comptyp, aiannhr, mtfcc, funcstat, aland, awater,
           intptlat, intptlon, geom
    FROM tiger2017.aiannh
...
CREATE TABLE tiger2017.aiannh250 AS (
    ...
    FROM
        (SELECT * FROM tiger2017.aiannh WHERE comptyp = 'R') r,
        (SELECT * FROM tiger2017.aiannh WHERE comptyp = 'T') t
    ...
    WHERE comptyp = 'R'
    AND aiannhce NOT IN (
        SELECT aiannhce FROM tiger2017.aiannh WHERE comptyp = 'T'
        )
