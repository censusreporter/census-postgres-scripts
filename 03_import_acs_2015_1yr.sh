...
# Create the schema
cd /home/ubuntu/census-postgres/acs2015_1yr
psql -d census -h $PGHOST -U census -c "DROP SCHEMA IF EXISTS acs2015_1yr CASCADE; CREATE SCHEMA acs2015_1yr;"
...
cat /mnt/tmp/acs2015_1yr/g20151*txt | psql -d census -h $PGHOST -U census -v ON_ERROR_STOP=1 -q -c "COPY acs2015_1yr.tmp_geoheader FROM STDIN WITH ENCODING 'latin1';"
if [[ $? != 0 ]]; then
...
do
    ...
    cat /mnt/tmp/acs2015_1yr/e20151[a-z][a-z]${s}*txt | psql -d census -h $PGHOST -U census -v ON_ERROR_STOP=1 -q -c "COPY acs2015_1yr.tmp_seq${s} FROM STDIN WITH CSV ENCODING 'latin1';"
    if [[ $? != 0 ]]; then
        echo "Failed importing sequences."
        exit 1
    fi
    cat /mnt/tmp/acs2015_1yr/m20151[a-z][a-z]${s}*txt | psql -d census -h $PGHOST -U census -v ON_ERROR_STOP=1 -q -c "COPY acs2015_1yr.tmp_seq${s}_moe FROM STDIN WITH CSV ENCODING 'latin1';"
    if [[ $? != 0 ]]; then
        echo "Failed importing sequences."
        exit 1
    fi
