...
-- and running them before 13_index_tiger_2024.sql and then moving the
-- census_name_lookup indexing to live alongside the related code in 13_index_tiger_2024
CREATE TABLE tiger2024.aiannh252 AS (
    ...
    FROM tiger2024.aiannh
    WHERE comptyp = 'R'
);

CREATE TABLE tiger2024.aiannh254 AS (
    SELECT gid, aiannhce, aiannhns, geoid, name, namelsad, lsad, classfp,
           comptyp, aiannhr, mtfcc, funcstat, aland, awater,
           intptlat, intptlon, geom
    FROM tiger2024.aiannh
...
CREATE TABLE tiger2024.aiannh250 AS (
    ...
    FROM
        (SELECT * FROM tiger2024.aiannh WHERE comptyp = 'R') r,
        (SELECT * FROM tiger2024.aiannh WHERE comptyp = 'T') t
    ...
    WHERE comptyp = 'R'
    AND aiannhce NOT IN (
        SELECT aiannhce FROM tiger2024.aiannh WHERE comptyp = 'T'
        )
...
INSERT INTO tiger2024.census_name_lookup
    SELECT
        ...
        aitsn.awater,
        aitsn.geom
    FROM tiger2024.aitsn LEFT OUTER JOIN acs2021_5yr.b01003 ON (('25100US' || aitsn.geoid) = b01003.geoid);
