CREATE TABLE tiger2020.aiannh252 AS (
    ...
    FROM tiger2020.aiannh
    WHERE comptyp = 'R'
);

CREATE TABLE tiger2020.aiannh254 AS (
    SELECT gid, aiannhce, aiannhns, geoid, name, namelsad, lsad, classfp,
           comptyp, aiannhr, mtfcc, funcstat, aland, awater,
           intptlat, intptlon, geom
    FROM tiger2020.aiannh
...
CREATE TABLE tiger2020.aiannh250 AS (
    ...
    FROM
        (SELECT * FROM tiger2020.aiannh WHERE comptyp = 'R') r,
        (SELECT * FROM tiger2020.aiannh WHERE comptyp = 'T') t
    ...
    WHERE comptyp = 'R'
    AND aiannhce NOT IN (
        SELECT aiannhce FROM tiger2020.aiannh WHERE comptyp = 'T'
        )
    ...
    WHERE comptyp = 'T'
    AND aiannhce NOT IN (
        SELECT aiannhce FROM tiger2020.aiannh WHERE comptyp = 'R'
        )
);

-- One row gives problems with the matching; fix that name manually
UPDATE tiger2020.aiannh250
