...
# Create the schema
cd /home/ubuntu/census-postgres/acs2016_1yr
psql -d census -h $PGHOST -U census -c "DROP SCHEMA IF EXISTS acs2016_1yr CASCADE; CREATE SCHEMA acs2016_1yr;"
...
cat /mnt/tmp/acs2016_1yr/g20161*txt | psql -d census -h $PGHOST -U census -v ON_ERROR_STOP=1 -q -c "COPY acs2016_1yr.tmp_geoheader FROM STDIN WITH ENCODING 'latin1';"
if [[ $? != 0 ]]; then
...
for s in $(seq -f "%04g" 1 166)
do
    echo "Importing sequence $s"
    cat /mnt/tmp/acs2016_1yr/e20161[a-z][a-z]${s}*txt | psql -d census -h $PGHOST -U census -v ON_ERROR_STOP=1 -q -c "COPY acs2016_1yr.tmp_seq${s} FROM STDIN WITH CSV ENCODING 'latin1';"
    if [[ $? != 0 ]]; then
        echo "Failed importing sequences."
        exit 1
    fi
    cat /mnt/tmp/acs2016_1yr/m20161[a-z][a-z]${s}*txt | psql -d census -h $PGHOST -U census -v ON_ERROR_STOP=1 -q -c "COPY acs2016_1yr.tmp_seq${s}_moe FROM STDIN WITH CSV ENCODING 'latin1';"
    if [[ $? != 0 ]]; then
        echo "Failed importing sequences."
        exit 1
    fi
